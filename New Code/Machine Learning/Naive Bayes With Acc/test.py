...
#    output[i] = unicode(output[i], "utf-8")
    ...
    isBenia = True
    isErab = True
    retulrWordWithoutTashkeel = removeTashkeel(result[i])
    outputWordWithoutTashkeel = removeTashkeel(output[i])
    ...
    for resultCounter in range(0, len(result[i])):
        ...
        while True:
            if allTashkeel.find(output[i][tempResultCounter]) == -1:
                break
        ...
        if resultChar == outputChar:
            ...
            if counter != (len(retulrWordWithoutTashkeel)-1):
                isBenia = False
            elif counter == (len(retulrWordWithoutTashkeel)-1):
                isErab= False
                break
            counter = counter + 1
